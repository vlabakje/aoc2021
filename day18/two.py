...
def add(n1, n2):
    ...
    tail = _below(n1, tail)
    tail.n = Node(",")
    tail = _below(n2, tail.n)
    tail.n = Node("]")
    print(f"after add: {nodes_to_str(head)}")
    return reduce(head)
...
def explode(head):
    ...
    while head.n:
        ...
        head = head.n
    if add_to_next != None:
        print(f"after explode: {nodes_to_str(orig)}")
...
def magnitude(head):
    ...
    while head.value == "[":
        while head.n:
            if (head.n.value == "[" and
                type(head.n.n.value) == int and
                head.n.n.n.value == "," and
                type(head.n.n.n.n.value) == int and
                head.n.n.n.n.n.value == "]"):
                # replace with result
                r = 3 * head.n.n.value + 2 * head.n.n.n.n.value
                head.n.value = r # = Node(value=r, n=head.n.n.n.n.n.n)
                head.n.n = head.n.n.n.n.n.n
            head = head.n
            break
        head = orig
    return head.value
...
def largest_magnitude(filename):
    ...
    for a, b in permutations(lines, 2):
        result = magnitude(add(str_to_nodes(a), str_to_nodes(b)))
        highest = max(result, highest)
    return highest

if __name__ == "__main__":
    print(largest_magnitude("input"))
