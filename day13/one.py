class Grid():
    ...
    def fold(self):
        axis, count = self._folds.pop(0)
        #print(f"folding along {axis=} {count=}")
        newdots = set()
        if axis == "y":
...
def visible_dots_single_fold(filename):
    grid = Grid(filename)
    grid.fold()
    return len(grid._dots)

if __name__ == "__main__":
    assert visible_dots_single_fold("example") == 17
    print(visible_dots_single_fold("input"))
